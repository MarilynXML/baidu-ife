...
function renderChart() {
    ...
    for(item in chartData){
        ...
        text+='<div class="chart_item" style="height: '+chartData[item]+'px ;background-color: '+color+'" title="'+item+': '+chartData[item]+'" onmouseover="hoverDetail(this)" onmouseout="hideDetail(this)"><span class="detail">date: '+item+'<br />num: '+chartData[item]+'</span></div>'
    }
...
function graTimeChange() {
  var ipt=formGraTime.getElementsByTagName("input");
    ...
    case "month":
      ...
      for(item in char){
        ...
        if(new Date(item).getMonth()!=month){
          ...
          if(sum!=0){
              chartData["2016年第"+month+"月"]=parseInt(sum/i);
            }
            sum=0;
            i=0;
          }  
